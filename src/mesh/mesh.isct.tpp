...
template <uint LEN>
inline IEptr find_edge(ShortVec<IEptr, LEN> &vec, Tptr key) {
  for (IEptr ie : vec) {
...
inline bool hasCommonVert(Tptr t0, Tptr t1) {
  return (t0->verts[0] == t1->verts[0] || t0->verts[0] == t1->verts[1] ||
          t0->verts[0] == t1->verts[2] || t0->verts[1] == t1->verts[0] ||
          t0->verts[1] == t1->verts[1] || t0->verts[1] == t1->verts[2] ||
          t0->verts[2] == t1->verts[0] || t0->verts[2] == t1->verts[1] ||
          t0->verts[2] == t1->verts[2]);
}

inline bool hasCommonVert(Eptr e, Tptr t) {
  return (e->verts[0] == t->verts[0] || e->verts[0] == t->verts[1] ||
          e->verts[0] == t->verts[2] || e->verts[1] == t->verts[0] ||
          e->verts[1] == t->verts[1] || e->verts[1] == t->verts[2]);
}
...
public:
  ...
  inline void init(IsctProblem *iprob, Tptr t) {
    ...
    for (uint k = 0; k < 3; k++) {
      oedges[k] = iprob->newOrigEdge(the_tri->edges[k], overts[(k + 1) % 3],
                                     overts[(k + 2) % 3]);
    }
  }
...
public:
  ...
  void addBoundaryEndpoint(IsctProblem *iprob, Tptr tri_key, Eptr edge,
                           IVptr iv) {
  ...
  IVptr addBoundaryEndpoint(IsctProblem *iprob, Tptr tri_key, Eptr edge,
                            Vec3d coord, GluePt glue) {
  ...
  IVptr addBoundaryPointAlone(IsctProblem *iprob, Eptr edge, Vec3d coord,
                              GluePt glue) {
    IVptr iv = iprob->newSplitIsctVert(coord, glue);
    addBoundaryHelper(edge, iv);
    return iv;
...
private:
  void subdivideEdge(IsctProblem *iprob, GEptr ge, ShortVec<GEptr, 8> &edges) {
    if (ge->interior.size() == 0) {
      ...
      SEptr se0 =
          iprob->newSplitEdge(ge->ends[0], ge->interior[0], ge->boundary);
      SEptr se1 =
          iprob->newSplitEdge(ge->interior[0], ge->ends[1], ge->boundary);
...
public:
  IsctProblem(Mesh *owner) : TopoCache(owner) {
    ...
    TopoCache::tris.for_each([](Tptr t) { t->data = nullptr; });
  ...
  inline void killOrigVert(OVptr ov) { ovpool.free(ov); }
  inline void killOrigEdge(OEptr oe) { oepool.free(oe); }
...
private:
  ...
  inline void marshallArithmeticInput(Empty3d::TriEdgeIn &input, Eptr e,
                                      Tptr t) const;
  inline void marshallArithmeticInput(Empty3d::TriTriTriIn &input, Tptr t0,
                                      Tptr t1, Tptr t2) const;
...
inline void for_pairs(ShortVec<T, LEN> &vec,
...
template <class VertData, class TriData>
inline void Mesh<VertData, TriData>::IsctProblem::for_edge_tri(
...
template <class VertData, class TriData>
inline GeomBlob<Eptr> Mesh<VertData, TriData>::IsctProblem::edge_blob(Eptr e) {
  GeomBlob<Eptr> blob;
...
template <class VertData, class TriData>
inline void Mesh<VertData, TriData>::IsctProblem::bvh_edge_tri(
    std::function<bool(Eptr e, Tptr t)> func) {
  std::vector<GeomBlob<Eptr>> edge_geoms;
  TopoCache::edges.for_each(
      [&](Eptr e) { edge_geoms.push_back(edge_blob(e)); });
...
void Mesh<VertData, TriData>::IsctProblem::findIntersections() {
  ...
  tprobs.for_each([&](Tprob tprob) { tprob->consolidate(this); });
}
...
template <class VertData, class TriData>
inline BBox3d Mesh<VertData, TriData>::IsctProblem::buildBox(Eptr e) const {
  Vec3d p0 = vPos(e->verts[0]);
  Vec3d p1 = vPos(e->verts[1]);
  return BBox3d(min(p0, p1), max(p0, p1));
}
template <class VertData, class TriData>
inline BBox3d Mesh<VertData, TriData>::IsctProblem::buildBox(Tptr t) const {
...
template <class VertData, class TriData>
inline void Mesh<VertData, TriData>::IsctProblem::marshallArithmeticInput(
    Empty3d::EdgeIn &input, Eptr e) const {
  input.p[0] = vPos(e->verts[0]);
  input.p[1] = vPos(e->verts[1]);
}
template <class VertData, class TriData>
inline void Mesh<VertData, TriData>::IsctProblem::marshallArithmeticInput(
    Empty3d::TriIn &input, Tptr t) const {
...
template <class VertData, class TriData>
bool Mesh<VertData, TriData>::IsctProblem::checkIsct(Tptr t0, Tptr t1,
                                                     Tptr t2) const {
...
template <class VertData, class TriData>
Vec3d Mesh<VertData, TriData>::IsctProblem::computeCoords(Eptr e,
                                                          Tptr t) const {
  Empty3d::TriEdgeIn input;
  marshallArithmeticInput(input, e, t);
...
template <class VertData, class TriData>
Vec3d Mesh<VertData, TriData>::IsctProblem::computeCoords(Tptr t0, Tptr t1,
                                                          Tptr t2) const {
  Empty3d::TriTriTriIn input;
  marshallArithmeticInput(input, t0, t1, t2);
  Vec3d coords = Empty3d::coordsExact(input);
...
template <class VertData, class TriData>
void Mesh<VertData, TriData>::IsctProblem::fillOutVertData(GluePt glue,
                                                           VertData &data) {
  ...
  } else if (glue->edge_tri_type) { // edge-tri type
...
template <class VertData, class TriData>
inline void Mesh<VertData, TriData>::subdivide_tri(uint t_piece_ref,
                                                   uint t_parent_ref) {
...
template <class VertData, class TriData>
void Mesh<VertData, TriData>::IsctProblem::fillOutTriData(Tptr piece,
                                                          Tptr parent) {
...
public:
  ...
  Eptr getTriangleEdge(GTptr gt, uint k, Tptr big_tri) {
    ...
    Eptr e = nullptr;
    if (typeid(gv0) == typeid(OVptr) && typeid(gv1) == typeid(OVptr)) {
...
void Mesh<VertData, TriData>::IsctProblem::resolveAllIntersections() {
  ...
  tprobs.for_each([&](Tprob tprob) { tprob->subdivide(this); });
  ...
  glue_pts.for_each([&](GluePt glue) { createRealPtFromGluePt(glue); });
  ...
  tprobs.for_each([&](Tprob tprob) { createRealTriangles(tprob, ecache); });

  // mark all edges as normal by zero-ing out the data pointer
  TopoCache::edges.for_each([](Eptr e) { e->data = 0; });
...
void Mesh<VertData, TriData>::testingComputeStaticIsct(
    std::vector<Vec3d> *points, std::vector<std::pair<Vec3d, Vec3d>> *edges) {
  IsctProblem iproblem(this);
