...
namespace corkpp {
  ...
  /*  to specify the regions in which one is traversing and modifying the
   * mesh.*/
  ...
  struct mesh {
    ...
    __int64
        undeads;   /* Number of input vertices that don't appear in the mesh. */
    ...
    __int64
        vertex2triindex; /* Index to find a triangle adjacent to a vertex. */
    __int64
  ...
  /*  long enough to hold a pointer are two of the few kludges in this
   * program.*/
  /*                                                                           */
...
#define decode(ptr, otri)                                                      \
  ...
  (otri).tri = (triangle *)((__int64)(ptr) ^ (__int64)(otri).orient)

  /* encode() compresses an oriented triangle into a single pointer.  It */
  /*   relies on the assumption that all triangles are aligned to four-byte */
  /*   boundaries, so the two least significant bits of (otri).tri are zero. */

#define encode(otri) (triangle)((__int64)(otri).tri | (__int64)(otri).orient)
...
#define otriequal(otri1, otri2)                                                \
  ...
  /*   the assumption that all subsegments are aligned to four-byte
   * boundaries.*/

#define infect(otri)                                                           \
  (otri).tri[6] = (triangle)((__int64)(otri).tri[6] | (__int64)2l)
...
#define infected(otri) (((__int64)(otri).tri[6] & (__int64)2l) != 0l)
...
#define sdecode(sptr, osub)                                                    \
  ...
  (osub).ss = (subseg *)((__int64)(sptr) & ~(__int64)3l)

  /* sencode() compresses an oriented subsegment into a single pointer.  It */
  /*   relies on the assumption that all subsegments are aligned to two-byte */
  /*   boundaries, so the least significant bit of (osub).ss is zero. */

#define sencode(osub) (subseg)((__int64)(osub).ss | (__int64)(osub).ssorient)
...
#define setmark(osub, value) *(__int64 *)((osub).ss + 8) = value
...
#define setvertexmark(vx, value) ((__int64 *)(vx))[m->vertexmarkindex] = value
...
#define setvertex2tri(vx, value) ((triangle *)(vx))[m->vertex2triindex] = value
  ...
  __int64 triunsuitable(vertex triorg, vertex tridest, vertex triapex,
                        REAL area)
#else  /* not ANSI_DECLARATORS */
  __int64 triunsuitable(triorg, tridest, triapex,
...
#else  /* not ANSI_DECLARATORS */
  void triexit(status) __int64 status;
#endif /* not ANSI_DECLARATORS */
...
#else  /* not ANSI_DECLARATORS */
  VOID * trimalloc(size) __int64 size;
#endif /* not ANSI_DECLARATORS */
...
#ifndef REDUCED
#ifndef CDT_ONLY
    printf("    -s  Force segments into mesh by splitting (instead of using "
           "CDT).\n");
#endif /* not CDT_ONLY */
  ...
  void info() {
    ...
    printf("triangulations, conforming Delaunay triangulations, Voronoi "
           "diagrams, and\n");
    printf("high-quality triangular meshes.  The latter can be generated with "
           "no small\n");
    printf("or large angles, and are thus suitable for finite element "
           "analysis.  If no\n");
    printf("command line switch is specified, your .node input file is read, "
           "and the\n");
    ...
    printf("-p switch is used.  If -r is used, you must supply .node and .ele "
           "files,\n");
    printf("and possibly a .poly file and an .area file as well.  The formats "
           "of these\n");
    ...
    printf("        constrained Delaunay triangulation (CCDT).  If you want a "
           "truly\n");
    printf("        Delaunay (not just constrained Delaunay) triangulation, "
           "use -D as\n");
    ...
    printf("    -r  Refines a previously generated mesh.  The mesh is read "
           "from a .node\n");
    printf("        file and an .ele file.  If -p is also used, a .poly file "
           "is read\n");
    printf("        and used to constrain segments in the mesh.  If -a is also "
           "used\n");
    printf("        (with no number following), an .area file is read and used "
           "to\n");
    printf("        impose area constraints on the mesh.  Further details on "
           "refinement\n");
    printf("        appear below.\n");
    printf("    -q  Quality mesh generation by Delaunay refinement (a hybrid "
           "of Paul\n");
    printf("        Chew's and Jim Ruppert's algorithms).  Adds vertices to "
           "the mesh to\n");
    printf(
    ...
    printf("        decimal point, but not exponential notation.  Note that a "
           "bound of\n");
    printf("        theta degrees on the smallest angle also implies a bound "
           "of\n");
    printf("        (180 - 2 theta) on the largest angle.  If the minimum "
           "angle is 28.6\n");
    printf("        degrees or smaller, Triangle is mathematically guaranteed "
           "to\n");
    ...
    printf("        an area constraint for each segment-bounded region, "
           "thereby\n");
    printf("        controlling triangle densities in a first triangulation of "
           "a PSLG.\n");
    printf("        You can impose both a fixed area constraint and a varying "
           "area\n");
    printf("        constraint by invoking the -a switch twice, once with and "
           "once\n");
    ...
    printf("        procedure in triangle.c to encode any constraint you like, "
           "then\n");
    printf("        recompile Triangle.  The other is to compile triangle.c "
           "with the\n");
    ...
    printf("        are triangulating a vertex set, this switch causes a .poly "
           "file to\n");
    printf("        be written, containing all edges of the convex hull.  If "
           "you are\n");
    ...
    printf("        triangulating a PSLG, Triangle assumes that you have "
           "identified the\n");
    printf("        region to be triangulated by surrounding it with segments "
           "of the\n");
    ...
    printf("        the introduction of an extremely thin angle between a PSLG "
           "segment\n");
    printf("        and a convex hull segment, which can cause overrefinement "
           "(and\n");
    printf("        possibly failure if Triangle runs out of precision).  If "
           "you are\n");
    ...
    printf("    -D  Conforming Delaunay triangulation:  use this switch if you "
           "want to\n");
    printf("        ensure that all the triangles in the mesh are Delaunay, "
           "and not\n");
    ...
    printf("    -j  Jettisons vertices that are not part of the final "
           "triangulation\n");
    printf("        from the output .node file.  By default, Triangle copies "
           "all\n");
    ...
    printf("        same order, so their indices do not change.  The -j switch "
           "prevents\n");
    printf("        duplicated input vertices, or vertices `eaten' by holes, "
           "from\n");
    printf("        appearing in the output .node file.  Thus, if two input "
           "vertices\n");
    printf("        have exactly the same coordinates, only the first appears "
           "in the\n");
    ...
    printf("    -B  No boundary markers in the output .node, .poly, and .edge "
           "output\n");
    printf("        files.  See the detailed discussion of boundary markers "
           "below.\n");
    ...
    printf("        files, so your input files won't be overwritten.  (If your "
           "input is\n");
    printf("        a .poly file only, a .node file is written.)  Cannot be "
           "used with\n");
    printf("        the -r switch, because that would overwrite your input "
           ".ele file.\n");
    printf("        Shouldn't be used with the -q, -a, -u, or -s switch if you "
           "are\n");
    ...
    printf("        arithmetic for certain tests if it thinks the inexact "
           "tests are not\n");
    printf("        accurate enough.  Exact arithmetic ensures the robustness "
           "of the\n");
    ...
    printf("        this switch is normally overridden by the value used to "
           "number the\n");
    printf("        first vertex of the input .node or .poly file.  However, "
           "this\n");
    ...
    printf("    -o2 Generates second-order subparametric elements with six "
           "nodes each.\n");
    printf("    -Y  No new vertices on the boundary.  This switch is useful "
           "when the\n");
    ...
    printf("        of the quality of the mesh; Triangle will try, but the "
           "resulting\n");
    printf("        mesh may contain poorly shaped triangles.  Works well if "
           "all the\n");
    ...
    printf("        angle and maximum area).  The default is to allow an "
           "unlimited\n");
    printf("        number.  If you specify this switch with no number after "
           "it,\n");
    printf("        the limit is set to zero.  Triangle always adds vertices "
           "at segment\n");
    ...
    printf("    -F  Uses Steven Fortune's sweepline algorithm to construct a "
           "Delaunay\n");
    printf("        triangulation.  Warning:  does not use exact arithmetic "
           "for all\n");
    ...
    printf("        default, Triangle alternates between vertical and "
           "horizontal cuts,\n");
    printf("        which usually improve the speed except with vertex sets "
           "that are\n");
    ...
    printf("    -s  Specifies that segments should be forced into the "
           "triangulation by\n");
    printf("        recursively splitting them at their midpoints, rather than "
           "by\n");
    ...
    printf("    -V  Verbose:  Gives detailed information about what Triangle "
           "is doing.\n");
    printf("        Add more `V's for increasing amount of detail.  `-V' is "
           "most\n");
    printf("        useful; itgives information on algorithmic progress and "
           "much more\n");
    ...
    printf("  vertices are the vertex set, that covers the convex hull of the "
           "vertex\n");
    printf("  set.  A Delaunay triangulation has the property that no vertex "
           "lies\n");
    ...
    printf("  A Voronoi diagram of a vertex set is a subdivision of the plane "
           "into\n");
    printf("  polygonal cells (some of which may be unbounded, meaning "
           "infinitely\n");
    printf("  large), where each cell is the set of points in the plane that "
           "are closer\n");
    ...
    printf("  A Planar Straight Line Graph (PSLG) is a set of vertices and "
           "segments.\n");
    printf("  Segments are simply edges, whose endpoints are all vertices in "
           "the PSLG.\n");
    printf("  Segments may intersect each other only at their endpoints.  The "
           "file\n");
    printf("  format for PSLGs (.poly files) is described below.\n\n");
    printf("  A constrained Delaunay triangulation (CDT) of a PSLG is similar "
           "to a\n");
    printf("  Delaunay triangulation, but each PSLG segment is present as a "
           "single edge\n");
    printf("  of the CDT.  (A constrained Delaunay triangulation is not truly "
           "a\n");
    printf("  Delaunay triangulation, because some of its triangles might not "
           "be\n");
    ...
    printf("  a linear contiguous sequence of edges of the triangulation.  New "
           "vertices\n");
    printf("  (not part of the PSLG) may appear, and each input segment may "
           "have been\n");
    ...
    printf("    If the fourth entry of the first line is `1', the last column "
           "of the\n");
    ...
    printf("    PSLG segments; a complete description appears in a section "
           "below.  The\n");
    printf("    .node file produced by Triangle contains boundary markers in "
           "the last\n");
    ...
    printf("    In .ele files produced by Triangle, each triangular element "
           "has three\n");
    printf(
    ...
    printf("    A .poly file represents a PSLG, as well as some additional "
           "information.\n");
    printf("    The first section lists all the vertices, and is identical to "
           "the\n");
    printf("    format of .node files.  <# of vertices> may be set to zero to "
           "indicate\n");
    printf("    that the vertices are listed in a separate .node file; .poly "
           "files\n");
    ...
    printf("    If -q, -a, -u, and -s are not selected, Triangle produces a "
           "constrained\n");
    printf("    Delaunay triangulation (CDT), in which each segment appears as "
           "a single\n");
    ...
    printf("    each hole.  After the triangulation is formed, Triangle "
           "creates holes\n");
    printf("    by eating triangles, spreading out from each hole point until "
           "its\n");
    ...
    printf("    eaten away.  If the two triangles abutting a segment are "
           "eaten, the\n");
    printf("    segment itself is also eaten.  Do not place a hole directly on "
           "a\n");
    printf(
    ...
    printf("    triangle area.  Triangle reads this section only if the -A "
           "switch is\n");
    printf("    used or the -a switch is used without a number following it, "
           "and the -r\n");
    printf("    switch is not used.  Regional attributes and area constraints "
           "are\n");
    printf("    propagated in the same manner as holes:  you specify a point "
           "for each\n");
    ...
    printf("    applied if the -A switch is selected), and the second value is "
           "assumed\n");
    printf("    to be a regional area constraint (but is only applied if the "
           "-a switch\n");
    ...
    printf("    When a triangulation is created from a .poly file, you must "
           "either\n");
    printf("    enclose the entire region to be triangulated in PSLG segments, "
           "or\n");
    printf("    use the -c switch, which automatically creates extra segments "
           "that\n");
    ...
    printf("    Triangle eats all triangles that are not enclosed by segments; "
           "if you\n");
    printf("    are not careful, your whole triangulation may be eaten away.  "
           "If you do\n");
    ...
    printf("    aren't required to make your .poly files ideal, but you should "
           "be aware\n");
    printf("    of what can go wrong.  Segment intersections are relatively "
           "safe--\n");
    printf("    Triangle calculates the intersection points for you and adds "
           "them to\n");
    printf("    the triangulation--as __int64 as your machine's floating-point "
           "precision\n");
    printf("    doesn't become a problem.  You are tempting the fates if you "
           "have three\n");
    printf("    segments that cross at the same location, and expect Triangle "
           "to figure\n");
    ...
    printf("    your output--unless Triangle tries to refine the tiny "
           "triangle, uses\n");
    printf("    up the last bit of machine precision, and fails to terminate "
           "at all.\n");
    printf("    You're better off putting the intersection point in the input "
           "files,\n");
    printf("    and manually breaking up each segment into two.  Similarly, if "
           "you\n");
    ...
    printf("    boundary).  The first neighbor of triangle i is opposite the "
           "first\n");
    ...
    printf("  vertices and edges occur on a boundary of the triangulation.  A "
           "common\n");
    printf("  use is to determine where boundary conditions should be applied "
           "to a\n");
    ...
    printf("      then it is assigned the same marker in the output .node "
           "file.\n");
    printf("    - Otherwise, if the vertex lies on a PSLG segment (even if it "
           "is an\n");
    ...
    printf("  and output files have iteration numbers.  For instance, Triangle "
           "might\n");
    printf("  read the files mesh.3.node, mesh.3.ele, and mesh.3.poly, refine "
           "the\n");
    printf(
    ...
    printf("  their iteration number is zero; hence, Triangle might read the "
           "file\n");
    printf("  points.node, triangulate it, and produce the files points.1.node "
           "and\n");
    printf("  points.1.ele.\n\n");
    ...
    printf("  being overwritten.  (If the input is a .poly file that contains "
           "its own\n");
    printf("  points, a .node file is written.  This can be quite convenient "
           "for\n");
    ...
    printf("  `triangle -pe object.1' reads a PSLG from object.1.poly (and "
           "possibly\n");
    printf("  object.1.node, if the vertices are omitted from object.1.poly) "
           "and writes\n");
    ...
    printf("     1   1.5 1.5\n");
    printf("\n");
    printf("  Note that some segments are missing from the outer square, so "
           "you must\n");
    printf("  use the `-c' switch.  After `triangle -pqc box.poly', here is "
           "the output\n");
    ...
    printf("  from segment 1.  Vertices 6 and 8 have markers from segment 4.  "
           "All the\n");
    printf("  other vertices but 4 have been marked to indicate that they lie "
           "on a\n");
    printf("  boundary.\n\n");
    printf("    12  2  0  1\n");
    printf("       1    0   0      5\n");
    ...
    printf("\n");
    printf("  The -r switch causes a mesh (.node and .ele files) to be read "
           "and\n");
    printf("  refined.  If the -p switch is also used, a .poly file is read "
           "and used to\n");
    ...
    printf("  When you refine a mesh, you generally want to impose tighter "
           "constraints.\n");
    printf("  One way to accomplish this is to use -q with a larger angle, or "
           "-a\n");
    printf("  followed by a smaller area than you used to generate the mesh "
           "you are\n");
    printf("  refining.  Another way to do this is to create an .area file, "
           "which\n");
    printf("  specifies a maximum area for each triangle, and use the -a "
           "switch\n");
    printf("  (without a number following).  Each triangle's area constraint "
           "is applied\n");
    printf("  to that triangle.  Area constraints tend to diffuse as the mesh "
           "is\n");
    printf("  refined, so if there are large variations in area constraint "
           "between\n");
    printf("  adjacent triangles, you may not get the results you want.  In "
           "that case,\n");
    printf("  consider instead using the -u switch and writing a C procedure "
           "that\n");
    ...
    printf("  element is contained in a single input element.  Often, an "
           "output element\n");
    printf("  can overlap two or three input elements, and some input edges "
           "are not\n");
    ...
    printf("  mesh of higher-order elements, the hierarchical property applies "
           "only to\n");
    printf("  the nodes at the corners of an element; the midpoint nodes on "
           "each edge\n");
    ...
    printf("  constraints in .area files are used only to refine an existing "
           "mesh, and\n");
    printf("  are typically based on a posteriori error estimates resulting "
           "from a\n");
    printf("  finite element simulation on that mesh.\n\n");
    printf("  `triangle -rq25 object.1' reads object.1.node and object.1.ele, "
           "then\n");
    ...
    printf("  The sequence `triangle -qa1 x', `triangle -rqa.3 x.1', `triangle "
           "-rqa.1\n");
    printf("  x.2' creates a sequence of successively finer meshes x.1, x.2, "
           "and x.3,\n");
    ...
    printf("  (including hole boundaries) as a by-product when you use the -c "
           "switch.\n");
    printf("  If you also use the -p switch, the output .poly file contains "
           "all the\n");
    ...
    printf("  .v.edge.  For example, `triangle -v points' reads points.node, "
           "produces\n");
    printf("  its Delaunay triangulation in points.1.node and points.1.ele, "
           "and\n");
    ...
    printf("  vertices, and does not check whether neighboring vertices are "
           "identical.\n");
    printf("  Be forewarned that if the Delaunay triangulation is degenerate "
           "or\n");
    printf("  near-degenerate, the Voronoi diagram may have duplicate vertices "
           "or\n");
    printf("  crossing edges.\n\n");
    printf("  The result is a valid Voronoi diagram only if Triangle's output "
           "is a true\n");
    printf("  Delaunay triangulation.  The Voronoi output is usually "
           "meaningless (and\n");
    printf("  may contain crossing edges and other pathology) if the output is "
           "a CDT or\n");
    printf("  CCDT, or if it has holes or concavities.  If the triangulated "
           "domain is\n");
    printf("  convex and has no holes, you can use -D switch to force Triangle "
           "to\n");
    ...
    printf("  wise from the Voronoi edge.  Triangle j of an .ele file is the "
           "dual of\n");
    printf("  vertex j of the corresponding .v.node file.  Voronoi cell k is "
           "the dual\n");
    printf("  of vertex k of the corresponding .node file.\n\n");
    printf("  Hence, to find the triangles adjacent to a Delaunay edge, look "
           "at the\n");
    printf("  vertices of the corresponding Voronoi edge.  If the endpoints of "
           "a\n");
    ...
    printf("  at the endpoints of the corresponding Delaunay edge.  If the "
           "endpoints of\n");
    printf("  a Delaunay edge are input vertices 7 and 12, then Voronoi cells "
           "7 and 12\n");
    ...
    printf("  Triangle does not write a list of the edges adjoining each "
           "Voronoi cell,\n");
    printf("  but you can reconstructed it straightforwardly.  For instance, "
           "to find\n");
    printf("  all the edges of Voronoi cell 1, search the output .edge file "
           "for every\n");
    printf("  edge that has input vertex 1 as an endpoint.  The corresponding "
           "dual\n");
    printf("  edges in the output .v.edge file form the boundary of Voronoi "
           "cell 1.\n");
    printf("\n");
    ...
    printf("  Triangle generates meshes with subparametric quadratic elements "
           "if the\n");
    ...
    printf("  rather than three.  `Subparametric' means that the edges of the "
           "triangles\n");
    printf("  are always straight, so that subparametric quadratic elements "
           "are\n");
    printf("  geometrically identical to linear elements, even though they can "
           "be used\n");
    printf("  with quadratic interpolating functions.  The three extra nodes "
           "of an\n");
    ...
    printf("  If two input segments adjoin each other at a small angle, "
           "clearly the -q\n");
    printf("  switch cannot remove the small angle.  Moreover, Triangle may "
           "have no\n");
    ...
    printf("  smaller than the specified bound.  However, these triangles only "
           "appear\n");
    printf("  between input segments separated by small angles.  Moreover, if "
           "you\n");
    ...
    printf("Statistics:\n\n");
    printf("  After generating a mesh, Triangle prints a count of entities in "
           "the\n");
    ...
    printf("  with the -rNEP switches to read the mesh and print the "
           "statistics without\n");
    printf("  writing any files.  Use -rpNEP if you've got a .poly file for "
           "the mesh.\n");
    printf("\n");
    ...
    printf("  histograms of the angles and the aspect ratios of the triangles "
           "in the\n");
    printf("  mesh.\n\n");
    ...
    printf("  most workstations do), and does not use extended precision "
           "internal\n");
    printf("  floating-point registers, then your output is guaranteed to be "
           "an\n");
    ...
    printf("  The exact tests can be disabled with the -X switch.  On most "
           "inputs, this\n");
    printf("  switch reduces the computation time by about eight percent--it's "
           "not\n");
    printf("  worth the risk.  There are rare difficult inputs (having many "
           "collinear\n");
    printf("  and cocircular vertices), however, for which the difference in "
           "speed\n");
    ...
    printf("    If you're using a PSLG, you've probably failed to specify a "
           "proper set\n");
    printf("    of bounding segments, or forgotten to use the -c switch.  Or "
           "you may\n");
    printf("    have placed a hole badly, thereby eating all your triangles.  "
           "To test\n");
    ...
    printf("    inside your vertex set, or even inside the densest part of "
           "your\n");
    printf("    mesh.  If you're triangulating an object whose x-coordinates "
           "all fall\n");
    ...
    printf("    such an angle is introduced by the -c switch.  If you don't "
           "realize\n");
    printf("    that a tiny angle is being formed, you might never discover "
           "why\n");
    printf("    Triangle is crashing.  To check for this possibility, use the "
           "-S switch\n");
    ...
    printf("    Zoom in closely, as such segments might look like a single "
           "segment from\n");
    printf("    a distance.\n\n");
    printf("    If some of the input values are too large, Triangle may suffer "
           "a\n");
    printf("    floating exception due to overflow when attempting to perform "
           "an\n");
    ...
    printf("    your input vertices has collinear vertices on its boundary, an "
           "input\n");
    printf("    vertex that you think lies on the convex hull might actually "
           "lie just\n");
    ...
    printf("    of your input vertices with a hole, or by placing them outside "
           "the area\n");
    printf("    enclosed by segments.  In any case, you can solve the problem "
           "by not\n");
    ...
    printf("    If you select the -X switch, Triangle occasionally makes "
           "mistakes due\n");
    printf("    to floating-point roundoff error.  Although these errors are "
           "rare,\n");
    ...
    printf("    If your input is a PSLG (-p), you can only expect a meaningful "
           "Voronoi\n");
    printf("    diagram if the domain you are triangulating is convex and free "
           "of\n");
    ...
    printf("  row of evenly spaced, segment-connected vertices?  Have you "
           "simply\n");
    printf("  defined one __int64 segment connecting the leftmost vertex to "
           "the rightmost\n");
    printf("  vertex, and a bunch of vertices lying along it?  This method "
           "occasionally\n");
    printf("  works, especially with horizontal and vertical lines, but often "
           "it\n");
    printf("  doesn't, and you'll have to connect each adjacent pair of "
           "vertices with a\n");
    printf("  separate segment.  If you don't like it, tough.\n\n");
    printf("  Furthermore, if you have segments that intersect other than at "
           "their\n");
    ...
    printf("  correctly for Triangle?  Are the triangles all listed so the "
           "first three\n");
    printf("  vertices are their corners in counterclockwise order?  Are all "
           "of the\n");
    ...
    printf("  purpose is to draw meshes on your screen or in PostScript.  Its "
           "secondary\n");
    printf("  purpose is to check the validity of your input files, and do so "
           "more\n");
    ...
    printf("A Brief Plea:\n");
    printf("\n");
    printf("  If you use Triangle, and especially if you use it to accomplish "
           "real\n");
    printf("  work, I would like very much to hear from you.  A short letter "
           "or email\n");
    printf("  (to jrs@cs.berkeley.edu) describing how you use Triangle will "
           "mean a lot\n");
    ...
    printf("  can justify spending time on improvements, which in turn will "
           "benefit\n");
    printf("  you.  Also, I can put you on a list to receive email whenever a "
           "new\n");
    ...
    printf("  Triangulator,'' in Applied Computational Geometry:  Towards "
           "Geometric\n");
    printf("  Engineering (Ming C. Lin and Dinesh Manocha, editors), volume "
           "1148 of\n");
    ...
    printf("  Of course, I can take credit for only a fraction of the ideas "
           "that made\n");
    printf("  this mesh generator possible.  Triangle owes its existence to "
           "the efforts\n");
    ...
    printf("  Walkington, and Binhai Zhu.  See the comments at the beginning "
           "of the\n");
...
#else  /* not ANSI_DECLARATORS */
  void parsecommandline(argc, argv, b) __int64 argc;
  char ** argv;
  struct behavior * b;
#endif /* not ANSI_DECLARATORS */
...
#define STARTINDEX 1
...

  {
    ...
    if (printvertex == (vertex)NULL)
      ...
      printf("    Origin[%d] = x%lx  (%.12g, %.12g)\n",
             (int)(t->orient + 1) % 3 + 3, (__int64)printvertex, printvertex[0],
             printvertex[1]);
    dest(*t, printvertex);
    if (printvertex == (vertex)NULL)
      printf("    Dest  [%d] = NULL\n", (int)(t->orient + 2) % 3 + 3);
      ...
      printf("    Dest  [%d] = x%lx  (%.12g, %.12g)\n",
             (int)(t->orient + 2) % 3 + 3, (__int64)printvertex, printvertex[0],
             printvertex[1]);
    apex(*t, printvertex);
    if (printvertex == (vertex)NULL)
      printf("    Apex  [%d] = NULL\n", (int)t->orient + 3);
      ...
      printf("    Apex  [%d] = x%lx  (%.12g, %.12g)\n", (int)t->orient + 3,
             (__int64)printvertex, printvertex[0], printvertex[1]);
...

  {
    ...
    if (printvertex == (vertex)NULL)
      ...
      printf("    Origin[%d] = x%lx  (%.12g, %.12g)\n",
             (int)2 + (int)s->ssorient, (__int64)printvertex, printvertex[0],
             printvertex[1]);
    sdest(*s, printvertex);
    if (printvertex == (vertex)NULL)
      printf("    Dest  [%d] = NULL\n", 3 - (int)s->ssorient);
    ...
    if (printvertex == (vertex)NULL)
      ...
      printf("    Segment origin[%d] = x%lx  (%.12g, %.12g)\n",
             4 + (int)s->ssorient, (__int64)printvertex, printvertex[0],
             printvertex[1]);
    ...
    if (printvertex == (vertex)NULL)
      ...
      printf("    Segment dest  [%d] = x%lx  (%.12g, %.12g)\n",
             5 - (int)s->ssorient, (__int64)printvertex, printvertex[0],
             printvertex[1]);
  }
...

  {
    ...
    pool->nextitem = (VOID *)(alignptr + (__int64)pool->alignbytes -
...

  {
    ...
    pool->firstblock =
        (VOID **)trimalloc(pool->itemsfirstblock * pool->itembytes +
                           (__int64)sizeof(VOID *) + pool->alignbytes);
...

  {
    ...
    pool->pathitem = (VOID *)(alignptr + (__int64)pool->alignbytes -
                              (alignptr % (__int64)pool->alignbytes));
...

  {
    ...
    m->dummytri = (triangle *)(alignptr + (__int64)m->triangles.alignbytes -
    ...
    if (b->usesegments) {
      ...
      m->dummysub = (subseg *)(alignptr + (__int64)m->subsegs.alignbytes -
...

  {
    ...
    m->vertexmarkindex =
        ((m->mesh_dim + m->nextras) * sizeof(REAL) + sizeof(__int64) - 1) /
        sizeof(__int64);
...
#define Two_One_Product(a1, a0, b, x3, x2, x1, x0)                             \
...
#ifdef ANSI_DECLARATORS
  __int64 fast_expansion_sum_zeroelim(__int64 elen, REAL * e, __int64 flen,
                                      REAL * f, REAL * h)
#else  /* not ANSI_DECLARATORS */
  __int64 fast_expansion_sum_zeroelim(elen, e, flen, f,
...
#else  /* not ANSI_DECLARATORS */
  __int64 scale_expansion_zeroelim(elen, e, b,
...
#else  /* not ANSI_DECLARATORS */
  REAL estimate(elen, e) __int64 elen;
  REAL * e;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
  REAL counterclockwise(struct mesh * m, struct behavior * b, vertex pa,
                        vertex pb, vertex pc)
#else  /* not ANSI_DECLARATORS */
...

  {
    ...
    __int64 axtbcttlen, aytbcttlen, bxtcattlen, bytcattlen, cxtabttlen,
        cytabttlen;
  ...
  void findcircumcenter(struct mesh * m, struct behavior * b, vertex torg,
                        ...
                        REAL * xi, REAL * eta, __int64 offcenter)
#else  /* not ANSI_DECLARATORS */
...
#else  /* not ANSI_DECLARATORS */
  __int64 randomnation(choices) __int64 choices;
#endif /* not ANSI_DECLARATORS */
...

  {
    ...
    if (b->verbose > 2) {
      ...
      printf("    (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)\n",
             ...
             badtri->triangdest[1], badtri->triangapex[0],
             badtri->triangapex[1]);
    }
  ...
  void enqueuebadtri(struct mesh * m, struct behavior * b, struct otri * enqtri,
                     REAL minedge, vertex enqapex, vertex enqorg,
                     vertex enqdest)
#else  /* not ANSI_DECLARATORS */
...
#else  /* not ANSI_DECLARATORS */
  __int64 checkseg4encroach(m, b, testsubseg) struct mesh * m;
  struct behavior * b;
  struct osub * testsubseg;
#endif /* not ANSI_DECLARATORS */
...

  {
    ...
    while (totalsamplesleft > 0) {
      ...
      firsttri = (char *)(alignptr + (__int64)m->triangles.alignbytes -
                          (alignptr % (__int64)m->triangles.alignbytes));

      /* Choose `samplesleft' randomly sampled triangles in this block. */
      do {
        sampletri.tri =
            (triangle *)(firsttri + (randomnation((__int64)population) *
...
#ifdef ANSI_DECLARATORS
  enum insertvertexresult
  insertvertex(struct mesh * m, struct behavior * b, vertex newvertex,
               struct otri * searchtri, struct osub * splitseg,
        ...
        if (brokensubseg.ss != m->dummysub) {
          ...
          if (segmentflaws) {
            ...
            if (enq) {
              ...
              if (b->verbose > 2) {
                printf("  Queueing encroached subsegment (%.12g, %.12g) "
                       "(%.12g, %.12g).\n",
          ...
          if (doflip) {
            ...
            if (b->vararea) {
              if ((areabound(top) <= 0.0) || (areabound(horiz) <= 0.0)) {
                ...
                /*   __int64, __int64 way from their original location due to
                 * flips. */
  ...
  /*  there's a lot of __int64 segments that each cut many triangles.  I ought
   * to */
...
#ifdef ANSI_DECLARATORS
  void vertexmedian(vertex * sortarray, __int64 arraysize, __int64 median,
                    __int64 axis)
#else  /* not ANSI_DECLARATORS */
  ...
  void divconqrecurse(struct mesh * m, struct behavior * b, vertex * sortarray,
                      __int64 vertices, __int64 axis, struct otri * farleft,
                      struct otri * farright)
#else  /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
  __int64 removeghosts(struct mesh * m, struct behavior * b,
                       struct otri * startghost)
#else  /* not ANSI_DECLARATORS */
  __int64 removeghosts(m, b, startghost) struct mesh * m;
  struct behavior * b;
  struct otri * startghost;
#endif /* not ANSI_DECLARATORS */
...
#else  /* not ANSI_DECLARATORS */
  __int64 divconqdelaunay(m, b) struct mesh * m;
  struct behavior * b;
#endif /* not ANSI_DECLARATORS */
...
#else  /* not ANSI_DECLARATORS */
  __int64 removebox(m, b) struct mesh * m;
  struct behavior * b;
#endif /* not ANSI_DECLARATORS */
...
#else  /* not ANSI_DECLARATORS */
  __int64 incrementaldelaunay(m, b) struct mesh * m;
  struct behavior * b;
#endif /* not ANSI_DECLARATORS */

  {
    ...
    while (vertexloop != (vertex)NULL) {
      ...
      if (insertvertex(m, b, vertexloop, &starttri, (struct osub *)NULL, 0,
                       0) == DUPLICATEVERTEX) {
        if (!b->quiet) {
          printf("Warning:  A duplicate vertex at (%.12g, %.12g) appeared and "
                 "was ignored.\n",
...
#ifdef ANSI_DECLARATORS
  void eventheapinsert(struct event ** heap, __int64 heapsize,
...

  {
    ...
    if (eventnum > 0) {
      ...
      do {
        parent = (eventnum - 1) >> 1;
        if ((heap[parent]->ykey < eventy) || ((heap[parent]->ykey == eventy) &&
...

  {
    ...
    *eventheap =
        (struct event **)trimalloc(maxevents * (__int64)sizeof(struct event *));
    *events =
        (struct event *)trimalloc(maxevents * (__int64)sizeof(struct event));
    traversalinit(&m->vertices);
    for (i = 0; i < m->invertices; i++) {
...
#ifdef ANSI_DECLARATORS
  __int64 rightofhyperbola(struct mesh * m, struct otri * fronttri,
                           vertex newsite)
#else  /* not ANSI_DECLARATORS */
  __int64 rightofhyperbola(m, fronttri, newsite) struct mesh * m;
  ...
  struct splaynode * splay(struct mesh * m, struct splaynode * splaytree,
                           vertex searchpoint, struct otri * searchtri)
#else  /* not ANSI_DECLARATORS */
  struct splaynode * splay(m, splaytree, searchpoint,
                           searchtri) struct mesh * m;
...
#else  /* not ANSI_DECLARATORS */
  struct splaynode * splayinsert(m, splayroot, newkey,
...
#else  /* not ANSI_DECLARATORS */
  __int64 sweeplinedelaunay(m, b) struct mesh * m;
  struct behavior * b;
#endif /* not ANSI_DECLARATORS */
...
#else  /* not ANSI_DECLARATORS */
  __int64 delaunay(m, b) struct mesh * m;
  struct behavior * b;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
  __int64 reconstruct(struct mesh * m, struct behavior * b,
                      __int64 * trianglelist, REAL * triangleattriblist,
                      REAL * trianglearealist, __int64 elements,
                      __int64 corners, __int64 attribs, __int64 * segmentlist,
  ...
  __int64 reconstruct(m, b, trianglelist, triangleattriblist, trianglearealist,
                      elements, corners, attribs, segmentlist,
                      segmentmarkerlist, numberofsegments) struct mesh * m;
  struct behavior * b;
...
#else  /* not ANSI_DECLARATORS */
  __int64 reconstruct(m, b, elefilename, areafilename, polyfilename,
...

  {
    ...
    vertexarray =
        (triangle *)trimalloc(m->vertices.items * (__int64)sizeof(triangle));
...
void segmentintersection(struct mesh * m, struct behavior * b,
                         ...
                         vertex endpoint2)
#else  /* not ANSI_DECLARATORS */
  void segmentintersection(m, b, splittri, splitsubseg,
                           endpoint2) struct mesh * m;
  struct behavior * b;
...
#ifdef ANSI_DECLARATORS
__int64 scoutsegment(struct mesh * m, struct behavior * b,
                     struct otri * searchtri, vertex endpoint2, __int64 newmark)
#else  /* not ANSI_DECLARATORS */
  __int64 scoutsegment(m, b, searchtri, endpoint2, newmark) struct mesh * m;
  struct behavior * b;
  struct otri * searchtri;
  vertex endpoint2;
  __int64 newmark;
...
#ifdef ANSI_DECLARATORS
void regionplague(struct mesh * m, struct behavior * b, REAL attribute,
                  REAL area)
#else  /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void carveholes(struct mesh * m, struct behavior * b,
                std::vector<REAL> holelist, __int64 holes,
                std::vector<REAL> regionlist, __int64 regions)
#else  /* not ANSI_DECLARATORS */
...

{
  ...
  if (regions > 0) {
    ...
    regiontris =
        (struct otri *)trimalloc(regions * (__int64)sizeof(struct otri));
  } else {
      ...
      if (!deadsubseg(currentenc.ss) && (eorg == encloop->subsegorg) &&
        ...
        if (b->verbose > 1) {
          printf("  Splitting subsegment (%.12g, %.12g) (%.12g, %.12g) at "
                 "(%.12g, %.12g).\n",
                 eorg[0], eorg[1], edest[0], edest[1], newvertex[0],
                 newvertex[1]);
        }
...
void transfernodes(struct mesh * m, struct behavior * b,
                   ...
                   std::vector<__int64> pointmarkerlist, __int64 numberofpoints,
...
void readholes(struct mesh * m, struct behavior * b, FILE * polyfile,
               char * polyfilename, REAL ** hlist, __int64 * holes,
...
#ifdef ANSI_DECLARATORS
void writenodes(struct mesh * m, struct behavior * b,
                std::vector<REAL> & pointlist,
                std::vector<REAL> & pointattriblist,
                std::vector<__int64> & pointmarkerlist)
#else  /* not ANSI_DECLARATORS */
    ...
    if (b->order == 1) {
      ...
      trianglelist[vertexindex++] = vertexmark(mid3);
        ...
        fprintf(outfile, "%4ld    %4d  %4d  %4d  %4d  %4d  %4d", elementnumber,
                ...
                vertexmark(mid1), vertexmark(mid2), vertexmark(mid3));
#endif /* not TRILIBRARY */
    }
...
void writepoly(struct mesh * m, struct behavior * b,
  ...
  void writepoly(struct mesh * m, struct behavior * b, char * polyfilename,
                 REAL * holelist, __int64 holes, REAL * regionlist,
                 __int64 regions, __int64 argc, char ** argv)
#else  /* not ANSI_DECLARATORS */
...
#ifdef TRILIBRARY
            edgemarkerlist[edgenumber - b->firstnumber] =
...
#ifdef ANSI_DECLARATORS
  void writeneighbors(struct mesh * m, struct behavior * b,
                      char * neighborfilename, __int64 argc, char ** argv)
#else  /* not ANSI_DECLARATORS */
...

{
  ...
  for (i = 0; i < 9; i++) {
    printf("    %3d - %3d degrees:  %8d    |    %3d - %3d degrees:  %8d\n",
           (int)i * 10, (int)i * 10 + 10, (int)angletable[i], (int)i * 10 + 90,
           (int)i * 10 + 100, (int)angletable[i + 9]);
  }
  printf("\n");
}
...
#else  /* not ANSI_DECLARATORS */
  __int64 main(argc, argv) __int64 argc;
  char ** argv;
#endif /* not ANSI_DECLARATORS */
